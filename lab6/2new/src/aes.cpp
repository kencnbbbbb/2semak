#include "aes.h"
#include "utils.h"
#include <iostream>
#include <iomanip>
#include <cstring>

static bool g_showState = false;

void AES128_SetShowState(bool value) {
    g_showState = value;
}

#define Nb 4
#define Nk 4
#define Nr 10

static const uint8_t sbox[256] = {
    
};

static const uint8_t inv_sbox[256] = {
    
};

static const uint8_t Rcon[11] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
};

static uint8_t xtime(uint8_t x) {
    return (x << 1) ^ (((x >> 7) & 1) * 0x1b);
}

static uint8_t Multiply(uint8_t x, uint8_t y) {
    uint8_t result = 0;
    while (y) {
        if (y & 1) result ^= x;
        x = xtime(x);
        y >>= 1;
    }
    return result;
}

static void PrintState(const char* title, uint8_t state[4][4]) {
    if (!g_showState) return;
    std::cout << title << std::endl;
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            std::cout << std::hex << std::setw(2) << std::setfill('0') << (int)state[i][j] << " ";
        }
        std::cout << std::endl;
    }
    std::cout << std::dec << std::endl;
}

// обычная замена байтов в матрице State через S-box
static void SubBytes(uint8_t state[4][4]) {
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            state[i][j] = sbox[state[i][j]];
}

static void InvSubBytes(uint8_t state[4][4]) {
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            state[i][j] = inv_sbox[state[i][j]];
}

// сдвиг строк матрицы State
static void ShiftRows(uint8_t state[4][4]) {
    uint8_t tmp;

    tmp = state[1][0];
    state[1][0] = state[1][1];
    state[1][1] = state[1][2];
    state[1][2] = state[1][3];
    state[1][3] = tmp;

    tmp = state[2][0];
    state[2][0] = state[2][2];
    state[2][2] = tmp;
    tmp = state[2][1];
    state[2][1] = state[2][3];
    state[2][3] = tmp;

    tmp = state[3][3];
    state[3][3] = state[3][2];
    state[3][2] = state[3][1];
    state[3][1] = state[3][0];
    state[3][0] = tmp;
}

static void InvShiftRows(uint8_t state[4][4]) {
    uint8_t tmp;

    tmp = state[1][3];
    state[1][3] = state[1][2];
    state[1][2] = state[1][1];
    state[1][1] = state[1][0];
    state[1][0] = tmp;

    tmp = state[2][0];
    state[2][0] = state[2][2];
    state[2][2] = tmp;
    tmp = state[2][1];
    state[2][1] = state[2][3];
    state[2][3] = tmp;

    tmp = state[3][0];
    state[3][0] = state[3][1];
    state[3][1] = state[3][2];
    state[3][2] = state[3][3];
    state[3][3] = tmp;
}

// смешивание столбцов State
static void MixColumns(uint8_t state[4][4]) {
    uint8_t Tmp, Tm, t;
    for (int i = 0; i < 4; i++) {
        t = state[0][i];
        Tmp = state[0][i] ^ state[1][i] ^ state[2][i] ^ state[3][i];
        Tm = state[0][i] ^ state[1][i];
        Tm = xtime(Tm);
        state[0][i] ^= Tm ^ Tmp;
        Tm = state[1][i] ^ state[2][i];
        Tm = xtime(Tm);
        state[1][i] ^= Tm ^ Tmp;
        Tm = state[2][i] ^ state[3][i];
        Tm = xtime(Tm);
        state[2][i] ^= Tm ^ Tmp;
        Tm = state[3][i] ^ t;
        Tm = xtime(Tm);
        state[3][i] ^= Tm ^ Tmp;
    }
}

static void InvMixColumns(uint8_t state[4][4]) {
    uint8_t a, b, c, d;
    for (int i = 0; i < 4; i++) {
        a = state[0][i];
        b = state[1][i];
        c = state[2][i];
        d = state[3][i];

        state[0][i] = Multiply(a, 0x0e) ^ Multiply(b, 0x0b) ^ Multiply(c, 0x0d) ^ Multiply(d, 0x09);
        state[1][i] = Multiply(a, 0x09) ^ Multiply(b, 0x0e) ^ Multiply(c, 0x0b) ^ Multiply(d, 0x0d);
        state[2][i] = Multiply(a, 0x0d) ^ Multiply(b, 0x09) ^ Multiply(c, 0x0e) ^ Multiply(d, 0x0b);
        state[3][i] = Multiply(a, 0x0b) ^ Multiply(b, 0x0d) ^ Multiply(c, 0x09) ^ Multiply(d, 0x0e);
    }
}

// XOR матрицы State с раундовым ключом
static void AddRoundKey(uint8_t round, uint8_t state[4][4], const uint8_t* roundKeys) {
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            state[j][i] ^= roundKeys[round * Nb * 4 + i * Nb + j];
}

void AES128_KeyExpansion(const uint8_t* key, uint8_t* roundKeys) {
    uint32_t i, j;
    uint8_t tempa[4];

    memcpy(roundKeys, key, Nk * 4);

    for (i = Nk; i < Nb * (Nr + 1); i++) {
        for (j = 0; j < 4; j++) {
            tempa[j] = roundKeys[(i - 1) * 4 + j];
        }

        if (i % Nk == 0) {
            uint8_t t = tempa[0];
            tempa[0] = tempa[1];
            tempa[1] = tempa[2];
            tempa[2] = tempa[3];
            tempa[3] = t;

            for (j = 0; j < 4; j++) {
                tempa[j] = sbox[tempa[j]];
            }

            tempa[0] ^= Rcon[i / Nk];
        }

        for (j = 0; j < 4; j++) {
            roundKeys[i * 4 + j] = roundKeys[(i - Nk) * 4 + j] ^ tempa[j];
        }
    }
}

void AES128_EncryptBlock(const uint8_t* input, const uint8_t* roundKeys, uint8_t* output) {
    uint8_t state[4][4];

    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            state[j][i] = input[i * 4 + j];

    PrintState("State до шифрования блока:", state);
    AddRoundKey(0, state, roundKeys);
    PrintState("State после AddRoundKey(0):", state);

    for (uint8_t round = 1; round < Nr; round++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(round, state, roundKeys);
        PrintState("State после очередного раунда:", state);
    }

    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(Nr, state, roundKeys);
    PrintState("State после финального раунда:", state);

    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            output[i * 4 + j] = state[j][i];
}

void AES128_DecryptBlock(const uint8_t* input, const uint8_t* roundKeys, uint8_t* output) {
    uint8_t state[4][4];

    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            state[j][i] = input[i * 4 + j];

    AddRoundKey(Nr, state, roundKeys);

    for (int round = Nr - 1; round >= 1; round--) {
        InvShiftRows(state);
        InvSubBytes(state);
        AddRoundKey(round, state, roundKeys);
        InvMixColumns(state);
    }

    InvShiftRows(state);
    InvSubBytes(state);
    AddRoundKey(0, state, roundKeys);

    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            output[i * 4 + j] = state[j][i];
}

void AES128_PrintRoundKeys(const uint8_t* roundKeys) {
    std::cout << "Все Round Keys:" << std::endl;
    for (int round = 0; round <= Nr; round++) {
        std::cout << "RoundKey " << round << ": ";
        for (int i = 0; i < 16; i++) {
            std::cout << std::hex << std::setw(2) << std::setfill('0')
                      << (int)roundKeys[round * 16 + i] << " ";
        }
        std::cout << std::dec << std::endl;
    }
    std::cout << std::endl;
}
